...
`ConversationMemory` 类负责将事件历史记录转换为结构化的对话格式，以便发送给大型语言模型（LLM）。它：

- 处理各种事件类型，包括用户消息、代理动作和系统观察
- 支持多模态内容处理，包括文本和图像
- 管理系统消息和用户消息
- 处理工具调用和响应
- 截断和格式化消息内容
...
- **事件到消息的转换**：将各种事件类型转换为 LLM 可以理解的消息格式
- **工具调用处理**：在函数调用模式下管理工具调用和响应
- **消息格式化**：应用格式化规则，如在连续的用户消息之间添加换行符
- **视觉内容处理**：处理图像 URL 和视觉浏览功能
...
2. **对话记忆流程**:
   - 代理从 State 请求对话历史
   - ConversationMemory 处理事件历史并将其转换为消息格式
   - 对于用户消息事件，创建用户角色消息
   - 对于代理动作事件，创建助手角色消息，可能包含工具调用
   - 对于系统观察事件，创建工具响应消息
   - 应用格式化规则并返回结构化消息列表
   - 代理使用这些消息构建 LLM 请求
